...
function LoadingContent() {
  ...
  // Add a ref for the connection timeout
  ...
  // Function to start question generation
  const startQuizGeneration = useCallback(async () => {
    // Prevent multiple requests
    if (generationStartedRef.current && !isRetrying) {
    ...
    try {
      ...
      sessionStorage.setItem("quiz_questions_total", questionCount.toString());
      ...
      // Set a timeout to detect connection issues early (3 seconds)
      if (connectionTimeoutRef.current) {
        clearTimeout(connectionTimeoutRef.current);
      }

      connectionTimeoutRef.current = setTimeout(() => {
        // If we haven't received any data after 3 seconds, assume connection issue
        if (progress.current === 0) {
          addDebugInfo("Connection timeout - no data received in 3 seconds");
          generationStartedRef.current = false;
          setError("Connection timeout. The LLM service might be unavailable.");
        }
      }, 3000);
  ...
  // If there's an error, show it with retry/cancel options
  if (error) {
    return (
